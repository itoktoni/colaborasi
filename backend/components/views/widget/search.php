            ...
            <form action="<?php echo $action; ?>" method="<?php echo $method; ?>">
                ...
                foreach ($field as $key => $item):
                    if (!is_array($item)):
                        ?>
                        <div class="col-md-3">
                            <div class="form-group is-empty"><input type="text" name="<?php echo $key; ?>" class="<?php echo $class; ?>" placeholder="Find <?= ucwords($key); ?>" value="<?php echo $item; ?>" id="<?= strtolower($key); ?>"><span class="material-input"></span></div>
                        </div>
                    <?php else: ?> 
                        ...
                        <?php elseif (isset($item['is_dropdown'])): ?>
                            <div class="col-md-3">
                                <select name="<?php echo (isset($item['name'])) ? $item['name'] : $key; ?>" class="<?php echo (isset($item['class'])) ? $item['class'] : 'form-control'; ?>">
                                    <option value="" selected><?php echo (isset($item['placeholder'])) ? ucwords($item['placeholder']) : 'Select ' . ucwords((isset($item['name'])) ? $item['name'] : $key); ?></option>
                                    <?php
                                    if (!isset($item['value'])):
                                    ...
                                    foreach ($item['item'] as $key => $e):
                                        ?>
                                        <option value="<?php echo $key; ?>" <?php echo ($item['value'] == $key) ? 'selected' : ''; ?>><?php echo $e; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <?php else: ?>
                            <div class="col-md-3">
                                <div class="form-group is-empty">
                                    <input type="text" name="<?php echo $item['name']; ?>" class="<?php echo $item['class']; ?>" placeholder="<?= $item['placeholder'] ?>" value="<?php echo (isset($item['value'])) ? $item['value'] : Yii::$app->request->get($item['name']); ?>" id="<?= strtolower($key); ?>">
                ...
                <div class="col-md-12  text-right">
                    <a href="<?php echo Url::to('/' . Yii::$app->request->pathinfo); ?>" class="btn btn-sm btn-round btn-default" rel="tooltip" title="Reset Pencarian"><i class="fas fa-sync-alt"></i></a>
